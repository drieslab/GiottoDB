...
dbs_polys <- dbSpatial::as_dbSpatial(rSpatial = gpolys[], conn = con, name = "gpolys")
dbs_points <- dbSpatial::as_dbSpatial(rSpatial = gpoints[], conn = con, name = "gpoints")

test_that("calculateOverlap: dbSpatial and spatVector matches", {
  # test Giotto
  res_sv <- GiottoClass::calculateOverlap( # TODO: update to .calculate_overlap_vector
    x = gpolys,
    y = gpoints
  )
  ...
  res_dbs_sv <- dbSpatial::vect(x = res_dbs, select = c('poly_ID', 'feat_ID', 'feat_ID_uniq'))
